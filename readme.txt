=== Quizlord ===
Contributors: Nahapet Nahapetyan - VMS studio 
Donate link: http://www.vms-studio.com/quizlord
Tags: quiz, quizzes, tests, questions, question taking, education, teaching, VMS studio,
Tested up to: 4.2.4
Stable tag: trunk
The most advanced quiz plugin for Wordpress. One plugin to rule the all. Quizlord features advanced possibilities not available in any other plugin.


== Description ==
This is a on-of-a-kind plugin. One of the biggest differences from any other plugin is the feature to stop and resume quizzes anytime. No more breaking of a big quiz into smaller bits and pieces, confusing the users and not letting them emerge into the quiz-taking process
Quizlord also offers a more traditional set of tools such as:Saving user scores in the databaseTime-limited quizzesEditing and removal of quizzesAdding, editing and removal of questions within any quiz
Statistics for each quiz from the admin panel
Loading of each question is achieved with Ajax, which makes everything clean, fast and efficient. It also offers higher security since it disables users from viewing the answers from within 'code inspectors', which is unforeseen in man other quiz plugins currently available.We also offer very fast support over emails or skype, as well as urgent bug fixes upon discovery. Contact us upon any questions or requests.


== Installation ==
1. Upload the QuizLord folder to the /wp-content/plugins/ directory
2. Activate the plugin through the 'Plugins' menu in WordPress

== Screenshots ==
1. Adding a quiz
2. Answering questions
3. Quiz statistics
4. Quiz list